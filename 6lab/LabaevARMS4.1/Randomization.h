...
public:
	...
	static const string getFackName() {
		int i = (rand() * 5) % (10 - 0);
		const string fackName = ranFackName[i];

		return fackName;
	}

	static const string getRanFIO() {
		int i = (rand() * 10) % (64 - 0);
		string FIO;
		const string name = ranName[i];
		const string surname = ranSurname[i];
	...
	static const string getDetailName() {
		int i = rand() % (10);
		const string detailName = ranDetailName[i];
